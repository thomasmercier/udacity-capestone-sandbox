...
W_fcN[0] = weight_variable([64, 4096])
b_fcN[0] = bias_variable([4096])
h_fcN[0] = tf.nn.softplus(tf.matmul(h_fc4, W_fcN[0]) + b_fcN[0])

for i in range(1, nFC):
    W_fcN[i] = weight_variable([4096, 4096])
    b_fcN[i] = bias_variable([4096])
    h_fcN[i] = tf.nn.softplus(tf.matmul(h_fcN[i-1], W_fcN[i]) + b_fcN[i])

W_fc5 = weight_variable([4096, 4096])
b_fc5 = bias_variable([4096])
h_fc5 = tf.nn.softplus(tf.matmul(h_fcN[nFC-1], W_fc5) + b_fc5)

decoded_img = tf.reshape(h_fc5, [-1, 64, 64, 1])
...
for i in range(300):
    ...
    a = loss.eval(feed_dict={img: batch})
    b = h_pool1_norm.eval(feed_dict={img: batch})
    c = h_concat1_norm.eval(feed_dict={img: batch})
    d = h_concat2_norm.eval(feed_dict={img: batch})
