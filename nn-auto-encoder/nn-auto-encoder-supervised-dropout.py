...
W_fcN[0] = weight_variable([16*16*64, 1024])
b_fcN[0] = bias_variable([1024])
h_fcN[0] = tf.nn.softplus(tf.matmul(h_flat1, W_fcN[0]) + b_fcN[0])

for i in range(1, nFC):
    W_fcN[i] = weight_variable([1024, 1024])
    b_fcN[i] = bias_variable([1024])
    h_fcN[i] = tf.nn.softplus(tf.matmul(h_fcN[i-1], W_fcN[i]) + b_fcN[i])

keep_prob = tf.placeholder(tf.float32)
h_fcN_drop = tf.nn.dropout(h_fcN[nFC-1], keep_prob)

W_fc1 = weight_variable([1024, 128])
b_fc1 = bias_variable([128])
h_fc1 = tf.nn.softplus(tf.matmul(h_fcN_drop, W_fc1) + b_fc1)
...
for i in range(1000):
    ...
    train_step.run(feed_dict={img: batch, theta:theta_, keep_prob: 0.5})
    a = loss.eval(feed_dict={img: batch, theta:theta_, keep_prob: 0.5})
    b = h_pool1_norm.eval(feed_dict={img: batch, theta: theta_, keep_prob: 0.5})
    c = h_concat1_norm.eval(feed_dict={img: batch, theta:theta_, keep_prob: 0.5})
    d = h_concat2_norm.eval(feed_dict={img: batch, theta:theta_, keep_prob: 0.5})
...
for i in range(N):
    ...
    param[i,:,:] = parameter.eval(feed_dict={img: batch, keep_prob: 1.0})

plt.plot(theta, param[:,0,0], 'r+')
plt.show()
