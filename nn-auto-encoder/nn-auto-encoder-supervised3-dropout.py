...
h_flat1 = tf.reshape(h_pool1, [-1, 32*32*size0])
...
W_fcN[0] = weight_variable([32*32*size0, size1])
...
h_fc2 = tf.nn.softplus(tf.matmul(h_fcN_drop, W_fc2) + b_fc2)
...
for i in range(M):
    ...
    train_step.run(feed_dict={img: batch, theta:theta_, keep_prob:1.0})
    a = loss.eval(feed_dict={img: batch, theta:theta_, keep_prob:1.0})
    print('%i -- %f' % (i, a))
