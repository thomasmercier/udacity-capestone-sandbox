...
h_conv2_1 = tf.nn.relu(tf.nn.conv2d(h_pool1, W_conv2_1, strides=[1,1,1,1], \
    padding='SAME') + b_conv2_1)
W_conv2_2 = weight_variable([3, 3, 4, 4])
b_conv2_2 = bias_variable([4])
h_conv2_2 = tf.nn.relu(tf.nn.conv2d(h_conv2_1, W_conv2_2, strides=[1,1,1,1], \
    padding='SAME') + b_conv2_2)
W_conv2_3 = weight_variable([1, 1, 4, 16])
b_conv2_3 = bias_variable([16])
h_conv2_3 = tf.nn.relu(tf.nn.conv2d(h_conv2_2, W_conv2_3, strides=[1,1,1,1], \
...
h_conv2_4 = tf.nn.relu(tf.nn.conv2d(h_pool1, W_conv2_4, strides=[1,1,1,1], \
    padding='SAME') + b_conv2_4)
W_conv2_5 = weight_variable([5, 5, 4, 4])
b_conv2_5 = bias_variable([4])
h_conv2_5 = tf.nn.relu(tf.nn.conv2d(h_conv2_4, W_conv2_5, strides=[1,1,1,1], \
    padding='SAME') + b_conv2_5)
W_conv2_6 = weight_variable([1, 1, 4, 16])
b_conv2_6 = bias_variable([16])
h_conv2_6 = tf.nn.relu(tf.nn.conv2d(h_conv2_5, W_conv2_6, strides=[1,1,1,1], \
...
h_conv2_7 = tf.nn.relu(tf.nn.conv2d(h_pool2, W_conv2_7, strides=[1,1,1,1], \
...
h_conv2_8 = tf.nn.relu(tf.nn.conv2d(h_pool1, W_conv2_8, strides=[1,1,1,1], \
...
h_conv3_1 = tf.nn.relu(tf.nn.conv2d(h_concat1, W_conv3_1, strides=[1,1,1,1], \
    padding='SAME') + b_conv3_1)
W_conv3_2 = weight_variable([3, 3, 16, 16])
b_conv3_2 = bias_variable([16])
h_conv3_2 = tf.nn.relu(tf.nn.conv2d(h_conv3_1, W_conv3_2, strides=[1,1,1,1], \
    padding='SAME') + b_conv3_2)
W_conv3_3 = weight_variable([1, 1, 16, 64])
b_conv3_3 = bias_variable([64])
h_conv3_3 = tf.nn.relu(tf.nn.conv2d(h_conv3_2, W_conv3_3, strides=[1,1,1,1], \
...
h_conv3_4 = tf.nn.relu(tf.nn.conv2d(h_concat1, W_conv3_4, strides=[1,1,1,1], \
    padding='SAME') + b_conv3_4)
W_conv3_5 = weight_variable([5, 5, 16, 16])
b_conv3_5 = bias_variable([16])
h_conv3_5 = tf.nn.relu(tf.nn.conv2d(h_conv3_4, W_conv3_5, strides=[1,1,1,1], \
    padding='SAME') + b_conv3_5)
W_conv3_6 = weight_variable([1, 1, 16, 64])
b_conv3_6 = bias_variable([64])
h_conv3_6 = tf.nn.relu(tf.nn.conv2d(h_conv3_5, W_conv3_6, strides=[1,1,1,1], \
...
h_conv3_7 = tf.nn.relu(tf.nn.conv2d(h_pool3, W_conv3_7, strides=[1,1,1,1], \
...
h_conv3_8 = tf.nn.relu(tf.nn.conv2d(h_concat1, W_conv3_8, strides=[1,1,1,1], \
...
#########################
# now we start decoding #
#########################

W_fc4 = weight_variable([2, 64])
b_fc4 = bias_variable([64])
h_fc4 = tf.matmul(parameter, W_fc4) + b_fc4

W_fc5 = weight_variable([64, 4096])
b_fc5 = bias_variable([4096])
...
W_fc8 = weight_variable([4096, 4096])
b_fc8 = bias_variable([4096])
h_fc8 = tf.nn.relu(tf.matmul(h_fc7, W_fc8) + b_fc8)

W_fc9 = weight_variable([4096, 4096])
b_fc9 = bias_variable([4096])
h_fc9 = tf.nn.relu(tf.matmul(h_fc8, W_fc9) + b_fc9)

decoded_img = tf.reshape(h_fc9, [-1, 64, 64, 1])
...
loss = tf.reduce_mean(tf.squared_difference(img, decoded_img))

train_step = tf.train.AdamOptimizer(1e-4).minimize(loss)
sess.run(tf.global_variables_initializer())
for i in range(1000):
    batch = image_batch(100)
    train_step.run(feed_dict={img: batch})
    print('%i -- %f' % (i, loss.eval(feed_dict={img: batch})))

N = 100
theta = np.linspace(0, 2*np.pi, N)
batch = np.empty( (N, 64, 64, 1), dtype=np.float32 )
for i in range(N):
    batch[i,:,:,0] = make_image(theta[i])
param = parameter.eval(feed_dict={img: batch})
plt.plot(theta, param)
plt.show()
