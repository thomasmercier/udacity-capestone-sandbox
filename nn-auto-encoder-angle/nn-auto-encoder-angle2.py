...
W_fcM[0] = weight_variable([image_size*image_size, size1])
b_fcM[0] = bias_variable([size1])
h_fcM[0] = tf.nn.relu(tf.matmul(h_fc2, W_fcN[0]) + b_fcN[0])

for i in range(1, nFC2):
    W_fcM[i] = weight_variable([size1, size1])
    b_fcM[i] = bias_variable([size1])
    h_fcM[i] = tf.nn.relu(tf.matmul(h_fcM[i-1], W_fcM[i]) + b_fcM[i])

W_fc3 = weight_variable([size1, image_size*image_size])
b_fc3 = bias_variable([image_size*image_size])
h_fc3 = tf.nn.relu(tf.matmul(h_fcM[nFC2-1], W_fc3) + b_fc3)
